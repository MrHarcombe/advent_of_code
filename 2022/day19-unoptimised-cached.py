...
def get_neighbours(state, blueprints, cache):
...
def depth_first(start_node, start_state, blueprints, cache={}):
    stack = []
    ...
    while len(stack) > 0:
        ...
        if len(current_path) >= MAX_DEPTH:
            ...
            for next_turn, next_state in get_neighbours(current_state, blueprints, cache):
