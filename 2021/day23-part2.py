...
def all_settled(chamber):
    ...
    for i, x in enumerate(['A', 'B', 'C', 'D']):
        if chamber[((2*(i+1))+1, 2)] != x or chamber[((2*(i+1))+1, 3)] != x or chamber[((2*(i+1))+1, 4)] != x or chamber[((2*(i+1))+1, 5)] != x:
            settled = False
...
def can_move(chamber, fella_pos):
    ...
    if fella_pos[1] > 1 and len(set([chamber[(fella_pos[0], i)] for i in range(2, fella_pos[1])]).difference(['.'])) != 0:
    ...
    elif fella_pos[0] == home_col and len(set([chamber[(fella_pos[0], i)] for i in range(fella_pos[1] + 1, 6)]).intersection([chamber[fella_pos]])) == 1:
        print('Not moving as in home column with friends underneath')
        return False

    # someone else in a home column
    elif fella_pos[1] == 1 and len(set([chamber[(home_col, i)] for i in range(2, 6)]).intersection([chamber[fella_pos]])) != 1:
