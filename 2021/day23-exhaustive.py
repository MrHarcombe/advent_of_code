...
def all_settled(chamber):
    for i, x in enumerate(['A', 'B', 'C', 'D']):
        if chamber[((2*(i+1))+1, 2)] != x or chamber[((2*(i+1))+1, 3)] != x:
            return False
...
def move_home(chamber, fella_pos):
    ...
    if clear_path(chamber, fella_pos, (home_col, 3)):
        # cost = follow_path(chamber, fella_pos, (home_col, 3)) * fella_cost
        return [(fella_pos, (home_col, 3))]

    elif ((chamber[fella_pos]) == chamber[(home_col, 3)]) and clear_path(chamber, fella_pos, (home_col, 2)):
        # cost = follow_path(chamber, fella_pos, (home_col, 2)) * fella_cost
...
def move_into_corridor(chamber, fella_pos):
    ...
    if home_col != 9:
        ...
        while not clear_path(chamber, fella_pos, tuple(ideal_destination)) and ideal_destination[0] <= 10:
            if ideal_destination[0] < 2:
                ideal_destination[0] += 1
        ...
        if clear_path(chamber, fella_pos, tuple(ideal_destination)):
            cost = follow_path(chamber, fella_pos, tuple(ideal_destination)) * fella_cost
            return True, cost
        ...
        while not clear_path(chamber, fella_pos, tuple(ideal_destination)) and ideal_destination[0] >= 2:
...
def can_move(chamber, fella_pos):
    ...
    if fella_pos[1] == 3 and chamber[(fella_pos[0], 2)] != '.':
        return False

    # already home on bottom
    elif fella_pos == (home_col, 3):
        return False

    # already home with friend underneath
    elif fella_pos == (home_col, 2) and ((chamber[fella_pos]) == chamber[(home_col, 3)]):
        return False

    # someone else in a home column
    elif fella_pos[1] == 1 and (chamber[(home_col, 2)] not in (chamber[fella_pos], '.') or chamber[(home_col, 3)] not in (chamber[fella_pos], '.')):
        return False

    # in corrdor and no clear path to home
    elif fella_pos[1] == 1 and not (clear_path(chamber, fella_pos, (home_col, 2))):
        return False
...
def find_moves(chamber):
    ...
    for pos in range(37):
        ...
        if chamber.get((x,y), '#') in ('A', 'B', 'C', 'D'):
            # print(f'found {chamber[(x,y)]} at {x,y}')
            if can_move(chamber, (x,y)):
                home_move = move_home(chamber, (x,y))
                if home_move: possible_moves += home_move
                corridor_moves = moves_into_corridor(chamber, (x,y))
                if corridor_moves: possible_moves += corridor_moves
...
costs = []
all_moves = [(find_moves(chamber), 0, chamber)]

while len(all_moves) > 0:
    moves, cost, chamber = all_moves.pop(0)
    for orig, dest in moves:
        new_cost, new_chamber = apply_move(chamber, orig, dest)
        if all_settled(new_chamber):
            costs.append(new_cost + cost)
        else:
            new_moves = find_moves(new_chamber)
            if new_moves:
                all_moves.append((new_moves, new_cost + cost, new_chamber))
