...
def resolve_addition(expr):
    op = expr.index("+")

    sub_total = total_subexpression(expr[op-1:op+2])

    return expr[:op-1] + [sub_total] + expr[op+2:]
...
def resolve_expression(expr):
    if "(" in expr:
        ...
        return resolve_expression(resolve_addition(expr))
