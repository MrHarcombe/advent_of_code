    ...
    while len(queue) > 0:
        ...
        for neighbour in step_up_to_neighbours(topomap, current[-1]):
            ...
            if topomap[neighbour] == 9:
                counts[start].add(tuple(next_path))
                continue
...
print("Part 1:", sum(len({p[-1] for p in paths}) for paths in counts.values()))
print("Part 2:", sum(len(paths) for paths in counts.values()))
