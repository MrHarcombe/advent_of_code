...
@cache
def build_grid_path(start, end, input_path, grid):
    output_path = ""
...
with StringIO(test) as input_data:
    ...
    for line in input_data:
        ...
        indirect_path = "A" + line_path

        for _ in range(25):
            for start, end in pairwise(indirect_path):
                # print("From", start, "to", end)
                path = refine_path(
                    indirect_grid,
                    dijkstra(
            ...
            complexity = len(indirect_path) * int(line.strip()[:-1])
            print(
                len(indirect_path),
                int(line.strip()[:-1]),
                len(indirect_path) * int(line.strip()[:-1]),
            )
