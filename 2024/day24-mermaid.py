...
with open("day24.mermaid", "w") as file:
    print("graph TD", file=file)
    for gate in gates.values():
        if isinstance(gate, ConstanteGate):
            print(f"{gate.name}([{gate.name} = {gate.get_value()}])", file=file)

        elif isinstance(gate, AndGate):
            print(f"{gate.name}[/{gate.name}/]", file=file)
            print(f"{gate.input1} --> {gate.name}", file=file)
            print(f"{gate.input2} --> {gate.name}", file=file)
