# Python3 program to find the longest cable length between any two cities.

# src - starting node for DFS traversal
# prev_len - sum of cable length till current node
# max_len - pointer which stores the maximum length of cable value after DFS traversal
...
def DFS(graph, src, prev_len, max_len, visited):
    ...
    for i in range(len(graph[src])):
        ...
        # If total cable length till now greater than previous
