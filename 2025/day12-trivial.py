...
def build_packages(counts, gifts):
    ...
    return combinations(parcels, sum(counts))
...
for size, region, counts in trees:
    for possible_packages in build_packages(counts, gifts):
        ...
        for package in possible_packages:
            spots += sum(package[0])
        # print(size, counts, spots)
        if min(size) < 3 or prod(size) < spots:
            # print("Trivial fail")
            break
        elif prod(size) // 9 > sum(counts):
            # print("Trivial success")
            region_count += 1
            break
        elif place_packages(size, region, possible_packages[0]):
            region_count += 1
            break
